...
def ListaComunidades(request):
    comunidades = Comunidad.objects.all()
    return render(request, 'core/lista_comunidades.html', {'comunidades':comunidades})

def AdministracionComunidades(request):
    eventos = Evento.objects.all()
    return render (request, 'core/administracion_comunidades.html', {'eventos':eventos})

def ListadoEventos(request):
    eventos = Evento.objects.all()
    return render(request, 'core/listado_eventos.html', {'eventos':eventos})    

def ListadoAgentesPastorales(request):
    agentes = Persona.objects.filter(tipo_persona__descripcion='Agente Pastoral')
    return render(request, 'core/lista_agentes_pastorales.html', {'agentes':agentes})
...
def ListadoEventosFiltroComunidad(request, comunidad):
    eventos = Evento.objects.filter(comunidad__pk=comunidad)
    return render(request, 'core/lista_eventos_filtro_comunidad.html', {'eventos':eventos})
